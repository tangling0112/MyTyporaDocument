...
```python
def convert_text2idx(exampels,word2inx):
    '''
    ...
    for sent in examples:
        idx_line.append(BOS)#在句子首部添加BOS开始符号
        for w in sent:
            if w in word2idx:
                idx_line.append(word2idx[w])
            else:
                idx_line.append(UNK)
        idx_line.append(EOS)#在句子尾部添加EOS句子结束符号
...
```python
def add_padding(idx_text,max_len):
    '''
...
```python
def read_en_corpus(src_path, max_len, lower_case=False):
   	'''src_path:数据集保存的文件地址
...
```python
import jieba
def read_zh_corpus(src_path, max_len):
   	'''src_path:数据集保存的文件地址
   	   max_len:读取的一句话可以包含的最多单词数
   	'''
    src_sents = []
...
```python
def build_En_vocab(examples, max_size, min_freq, extra_tokens):
...
```python
import Counter
def buld_zh_vocab(examples,max_size,min_freq,extra_tokens)
	'''
